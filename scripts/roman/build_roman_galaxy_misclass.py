...
CACHE_OUT = (
    ROMAN_DIR / "roman_lsst_matched.parquet"
)  # the merged table doubles as the cache
...
def load_true_galaxy_sizes():
    ...
    if not COSMODC2_SIZE_FILE.exists():
        return None
    df = pd.read_parquet(
        COSMODC2_SIZE_FILE, columns=[COSMODC2_SIZE_ID_COL, COSMODC2_SIZE_VALUE_COL]
    )
    return df.rename(
        columns={
            COSMODC2_SIZE_ID_COL: "cosmodc2_id",
...
def resolve_galaxy_size(merged, size_source):
    ...
    if size_source == "measured_roman":
        ...
        return (
            np.asarray(sz),
            'measured Roman F158 size_sb = sqrt(lambda1)*3600" (INTERIM proxy; PSF~0.27")',
        )
    ...
    if size_source == "cosmodc2_true":
        ...
        lut = dict(
            zip(true_sizes["cosmodc2_id"].values, true_sizes["size_true"].values)
        )
        sz = np.array([lut.get(k, np.nan) for k in key], dtype=float)
        return (
            sz,
            f"cosmoDC2 true size joined by cosmodc2_id from {COSMODC2_SIZE_FILE.name}",
        )
...
def read_roman_bbox(dset, ra_lo, ra_hi, dec_lo, dec_hi):
    ...
    f = pads.field
    filt = (
        (f("alphawin_j2000") >= ra_lo)
        & (f("alphawin_j2000") <= ra_hi)
        & (f("deltawin_j2000") >= dec_lo)
        & (f("deltawin_j2000") <= dec_hi)
    )
...
def main():
    ...
    print(
        f"REF_MAGLIM (DC2 F158 maglim-map median) = {REF_MAGLIM:.4f}  "
    ...
    if MERGED_OUT.exists() and not args.force:
        ...
        obj_files = sorted(
            glob(str(LSST_DIR / "dc2_object_run2.2i_dr6_skim_tract_*.fits"))
        )
        truth_files = {
            Path(f).name.split("_")[-1].split(".")[0]: f
            for f in glob(
                str(LSST_DIR / "dc2_run2.2i_truth_merged_summary_skim_tract_*.fits")
            )
        }
        ...
        for obj_file in obj_files:
            ...
            dec_lo, dec_hi = (
                lsst_obs["dec"].min() - margin,
                lsst_obs["dec"].max() + margin,
            )
            roman = read_roman_bbox(dset, ra_lo, ra_hi, dec_lo, dec_hi)
            n_roman_seen += len(roman)
            if len(roman) == 0 or len(lsst_obs) == 0:
                print(
                    f"  tract {tract}: roman={len(roman)} lsst_obj={len(lsst_obs)} -> 0 matches"
                )
                continue
            ...
            c_roman = SkyCoord(
                ra=roman["alphawin_j2000"].values * u.deg,
                dec=roman["deltawin_j2000"].values * u.deg,
            )
            c_obj = SkyCoord(
                ra=lsst_obs["ra"].values * u.deg, dec=lsst_obs["dec"].values * u.deg
            )
            ...
            c_true = SkyCoord(
                ra=lsst_true["ra"].values * u.deg, dec=lsst_true["dec"].values * u.deg
            )
            c_objm = SkyCoord(
                ra=lsst_obs_m["ra"].values * u.deg, dec=lsst_obs_m["dec"].values * u.deg
            )
            ...
            print(
                f"  tract {tract}: roman_bbox={len(roman)+0:,} lsst_obj={len(lsst_obs):,} "
        ...
        before = len(merged)
        merged = (
            merged.sort_values("roman_lsst_sep_arcsec")
        ...
        print(
            f"saved merged table -> {MERGED_OUT} ({len(merged):,} rows, "
    ...
    fit_cat = dset.to_table(
        columns=fit_cols, filter=pads.field("matched") == True
    ).to_pandas()
    clf = build_env_classifier(fit_cat)
    classify_star = clf.classify
    print(
        f"  size-envelope classifier: Delta@21={float(clf.Dfun(21)):.3f} "
    ...
    gal = gal.sort_values("roman_obs_match_sep_arcsec").drop_duplicates(
        "roman_true_ind"
    )
    ...
    print(
        f'compact (size<{GAL_SIZE_MAX}") true galaxies with truth mag: {int(sel.sum()):,}'
    )
    ...
    for _, r in full[full.n_gal >= 20].iterrows():
        print(
            f"  F158={r.mag_F158:6.3f}  delta={r.delta_mag:+6.3f}  "
