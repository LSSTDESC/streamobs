...
for b in ("r", "g"):
    sv.sys_error[b] = 0.0
    curves[b] = np.array(
        [float(np.atleast_1d(sv.get_photo_error(b, ml + x, ml, kind="catalog"))[0]) for x in d]
    )
...
a.plot(d, curves["g"], color="C3", lw=2.2, ls="--", label="g  (forced photometry -> _nocut curve)")
a.axhline(0.1000, color="k", ls=":", lw=1.4)
a.axhline(K / 10, color="green", ls="-.", lw=1.4)
a.axvline(PROBE, color="0.5", lw=1.0)
a.annotate("magerr = 0.1000\n(what the test's 1/err == 10 implies)",
           xy=(-1.95, 0.1000), xytext=(-1.93, 0.113), fontsize=8, color="k")
a.annotate(f"magerr = {K/10:.5f}\n(TRUE S/N = 10)",
           xy=(-1.95, K / 10), xytext=(-1.93, 0.083), fontsize=8, color="green")
a.plot([PROBE], [e_r], "o", color="C0", ms=7, zorder=5)
a.plot([PROBE], [e_g], "o", color="C3", ms=7, zorder=5)
a.set(xlabel=r"$\Delta m = m - {\rm maglim}$", ylabel=r"catalog $\sigma$ (mag)",
      xlim=(-2, 0), ylim=(0.02, 0.16),
...
b_.plot(d, K / curves["r"], color="C0", lw=2.2, label="r: TRUE S/N $=2.5/\\ln 10/\\sigma$")
b_.plot(d, K / curves["g"], color="C3", lw=2.2, ls="--", label="g: TRUE S/N")
b_.plot(d, 1 / curves["r"], color="C0", lw=1.6, alpha=0.55, ls=":",
        label="r: what the test computes ($1/\\sigma$)")
b_.plot(d, 1 / curves["g"], color="C3", lw=1.6, alpha=0.55, ls=":",
...
b_.annotate(f"TRUE S/N: r={K/e_r:.2f}, g={K/e_g:.2f}  PASS",
            xy=(PROBE, K / e_r), xytext=(-1.95, 11.6), fontsize=9, color="green")
b_.annotate(f"1/$\\sigma$: r={1/e_r:.3f}, g={1/e_g:.3f}  FAIL",
            xy=(PROBE, 1 / e_g), xytext=(-1.95, 8.1), fontsize=9, color="firebrick")
b_.set(xlabel=r"$\Delta m = m - {\rm maglim}$", ylabel="S/N",
       xlim=(-2, 0), ylim=(4, 14),
       title="The 1.0857 factor is the whole failure")
b_.legend(fontsize=7.5, loc="lower left"); b_.grid(alpha=0.3)

fig.suptitle(
    "PR #67 S/N=10 assertion: the curves are correct, the S/N conversion is not "
    f"(det_ok vs _nocut differ by only {100*(e_g/e_r-1):.1f}% here)", fontsize=11)
fig.tight_layout()
