...
# <name>_<band>_true columns, keyed on the survey *name* only — the release is
# dropped, since a true magnitude does not depend on which release observed it.
# Here the survey is lsst/yr1, so the prefix is "lsst", not "lsst_yr1".
...
observed/error/flag columns carry the full `{name}_{release}` namespace — for the
LSST/yr1 survey loaded above, `lsst_yr1` — while true-magnitude columns are keyed
on the survey **name** only, with the release dropped. See
[Output column convention](column_convention.md)):
- `ra`, `dec`: Sky coordinates
- `lsst_g_true`, `lsst_r_true`: True (noiseless) apparent magnitudes (name only — no release)
